...
namespace ECSFlowField.Helpers
{
    public class FlowFieldHelper : MonoBehaviour
    {
        ...
        public static int FindCellBufferIndex(DynamicBuffer<FlowFieldCellPositionBufferElement> flowFieldCellPositionBuffer, int3 index)
        {
        ...
        private void OnDrawGizmos()
        {
            ...
            var positionBuffer = positionsBufferElement.Reinterpret<FFPosition>();
            var costBuffer = costBufferElement.Reinterpret<FFCost>();
            var directionBuffer = directionBufferElement.Reinterpret<FFDirection>();
            ...
            for (var i = 0; i < flowFieldComponent.CellCount; i++)
            {
                ...
                var bestDirection = directionBuffer[i].Value;
