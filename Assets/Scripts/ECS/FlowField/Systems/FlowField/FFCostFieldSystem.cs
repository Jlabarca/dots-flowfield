...
using TopDownCharacterController.Project.Scripts.ECS.SystemsAndJobs.FlowField;
using Unity.Burst;
...
{
    /// <summary>
    /// Costfield calculated using parallel raycast jobs on every cell
    /// </summary>
    [UpdateAfter(typeof(FFCellBufferInitializationSystem))]
    [UpdateAfter(typeof(FFCalculateCellPositionsSystem))]
    [UpdateInGroup(typeof(FlowFieldSystemGroup)), DisableAutoCreation]
    public partial class FFCostFieldSystem : SystemBase
    {
        ...
        {
            if (_endInitializationEntityCommandBufferSystem is null)
            {
                GetDependencies();
                return;
            }
            ...
            Entities
                ...
                {
                    ecb.RemoveComponent<FFGetCostFieldTag>(e);
                    ...
                    _stepPhysicsWorld.FinalSimulationJobHandle.Complete(); // Complete FinalSimulationJob before ray casting on multiple threads
                    var flowFieldRaycastJobHandle =
                        ScheduleFlowFieldRaycastJob(_buildPhysicsWorld.PhysicsWorld.CollisionWorld, cellPositionBuffer, cellRadius, raycastInputs, ref raycastHitResults,
                            _stepPhysicsWorld.FinalSimulationJobHandle);
                    flowFieldRaycastJobHandle.Complete();
                    ...
                    {
                        var cellWorldPos = cellPositionBuffer[i].Value.WorldPos;
                        var cellFlowFieldIndex = cellPositionBuffer[i].Value.FlowFieldCellIndex;
                        ...
                        var tmpCellCostComponent = cellCostBuffer[flatIndex].Value;
                        tmpCellCostComponent.Value = cellCost;
        ...
        private JobHandle ScheduleFlowFieldRaycastJob(
            CollisionWorld collisionWorld, DynamicBuffer<FlowFieldCellPositionBufferElement> cellPositionsBuffer, float cellRadius, NativeArray<RaycastInput> inputs, ref NativeArray<RaycastHit> results,
            JobHandle cellWorldPositionsCalcJobHandle)
        {
            var collisionFilter = new CollisionFilter
            {
                BelongsTo = ~0u,
            ...
            {
                var worldPos = cellPositionsBuffer[i].Value.WorldPos;
        ...
        private struct FlowFieldCellCostEvaluationJob : IJobParallelFor
        {
            ...
            [ReadOnly] public DynamicBuffer<FlowFieldCellPositionBufferElement> PositionsBuffer;

            public NativeArray<(float3, byte)> CostResults;

            public void Execute(int index)
            {
                var worldPos = PositionsBuffer[index].Value.WorldPos;
